...
<script>
import { getCharacterList, delCharacterModel, getCharacterModel } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    async handleClickModel(row) {
      ...
      const { data: res } = await getCharacterModel(row._id);
    ...
    handleClickEditOrDel(row, id) {
      if (id == 0) {
        this.dialogOff();
        this.dialogTitle = "编辑装备";
        this.categoryModel = row;
        this.dialogType = false;
      } else {
        this.$confirm(`此操作将永久删除该装备${row.name}，是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'error'
        }).then(async () => {
          await delCharacterModel(row._id);
          this.$message({
            type: 'success',
            message: '删除成功！'
          });
